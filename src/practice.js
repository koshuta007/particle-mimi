...
function questionHTML(R){
  ...
  if(!q.answered){
    ...
    if(q.verdict==="ok")ctl+='<div class="verdict verdict-ok">'+ico("check",16)+'<span>Вярно.</span>'+moveHTML(q.move)+'</div>';
      ...
      ctl+='<div class="verdict verdict-no">'+ico("x",16)+'<span>'+(q.given?'Ти: <b>'+esc(q.given)+'</b>. ':"")+'Търсеше се <b>'+esc(v.phrasal)+'</b> — '+esc(v.formal)+'.</span>'+moveHTML(q.move)+'</div>';
...
function summaryHTML(s){
  const rows=s.results.map((r,k)=>{
    ...
    return '<div class="mis"><div class="mis-n">'+(k+1)+'</div><div class="mis-body"><div class="mis-head">'+ic+' '+pvHTML(v)
      ...
      +(r.verdict!=="ok"?'<div class="mis-you">ти: '+(r.given?esc(r.given):"празно")+(r.matched?' — '+esc(r.matched)+' е синоним, картата не мърда':"")+'</div>':"")
